...
export default {
  ...
  setup () {
    ...
    const todos = computed(() => store.state.todomvc.todos)
    ...
    const toggleAll = (done) => store.dispatch('todomvc/toggleAll', done)
    const clearCompleted = () => store.dispatch('todomvc/clearCompleted')

    function addTodo (e) {
      const text = e.target.value
      if (text.trim()) {
        store.dispatch('todomvc/addTodo', text)
      }
      e.target.value = ''
    }
